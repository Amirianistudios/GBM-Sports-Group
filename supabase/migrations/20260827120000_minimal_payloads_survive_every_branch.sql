-- ============================================================================
-- GBM INTELLIGENCE — 0031 A MINIMAL PAYLOAD MUST SURVIVE EVERY BRANCH
-- ----------------------------------------------------------------------------
-- 0029 and 0030 each fixed one instance of the same fault: a column that is
-- `not null default <x>` handed an explicit NULL, because the function names
-- the column and the payload omits the field. Naming a column in an INSERT
-- and giving it NULL does not fall back to the DEFAULT.
--
-- Rather than wait to trip over a fourth, every not-null column these branches
-- write was checked against the expression supplying it. One more was found,
-- and two required fields were producing an unhelpful error:
--
--   1. `source_facts.confidence` is `not null default 0.800`, and the FACT
--      branch passed `(v_data->>'confidence')::numeric` — NULL whenever the
--      payload omits it. Every FACT submission without an explicit confidence
--      was refused. Now coalesced to the column's own default.
--
--   2. `player_news.source_name` and `source_facts.fact_key` are not null and
--      have NO default, so they are genuinely required. They were failing on
--      the constraint and surfacing as WRITE_FAILED with a Postgres message.
--      Neither may be invented — a news item with a made-up source is worse
--      than a rejected one — so both are now refused by name, with a reason.
--
-- The remaining not-null columns are safe: player_id, agent_id, entity_type
-- and provider_code come from the caller's identity or a coalesce; headline,
-- report_type, recommendation, content_hash, sections and sources already
-- coalesce; is_current, version, created_at and retrieved_at take defaults or
-- literals.
--
-- Values that violate a CHECK or an enum still return WRITE_FAILED carrying
-- sqlerrm and sqlstate, which names the constraint — that is a machine-readable
-- answer, so it is left alone rather than duplicated as validation here.
-- ============================================================================
...
-- The guard
-- ----------------------------------------------------------------------------
-- Re-reads the installed definition and asserts each of the three fixes is
-- present, so applying this on top of a hand-edited function fails loudly.
-- ----------------------------------------------------------------------------
do $guard$
declare
  v_def text;
...
begin
  ...
  if v_def !~ 'coalesce\(\(v_data->>''confidence''\)::numeric, 0.800\)' then
    v_missing := v_missing || 'source_facts.confidence is not coalesced to its default; ';
  end if;
  if v_def !~ 'MISSING_REQUIRED_FIELD' then
    v_missing := v_missing || 'the required-field rejections are absent; ';
  end if;
  if v_def ~ 'v_data->''advanced'', now\(\)' then
    v_missing := v_missing || '0030''s advanced fix has been lost; ';
  end if;

  if v_missing <> '' then
    raise exception 'gbm_intel_submit() is not in the expected state: %', v_missing;
  end if;
end $guard$;

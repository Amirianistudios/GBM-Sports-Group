...
`DATA_SOURCE_RESEARCH.md`; the table below maps the brief's list onto those
verdicts and the current state of each integration:

| # | Brief's source | Status after audit |
|---|---|---|
| 1 | `dcaribou/transfermarkt-datasets` | **LIVE — already integrated.** The download/import pipeline is built against its published R2 bucket; local snapshot used for development was release 2026-08-05 (12 tables, ~228 MB): 50,149 players, 796 clubs, 65 competitions, 175,165 transfers, 656,301 valuations. Weekly-refresh cadence assumed by the seeded `transfermarkt_dataset_update` job (Tuesdays). CC0-style open data derived from Transfermarkt pages; zero youth competitions — it can never surface a youth player. |
| 2 | `felipeall/transfermarkt-api` | **Research only, as the brief instructs.** MIT, self-hostable live scraper of Transfermarkt pages. Assessed indirectly during the SportDB investigation (SportDB resells exactly this route table). Useful as endpoint documentation; building GBM's pipeline on live Transfermarkt scraping would create exactly the fragile dependency the dataset route avoids. The dataset (#1) already covers players, clubs, competitions, transfers, market values with none of the scraping risk. No adapter warranted now. |
| 3 | `eddwebster/football_analytics` | **Methods reference, not a data source.** A large personal corpus of notebooks and write-ups on player evaluation, metrics and dashboards. Mine it when designing the analytics layer (per-90s, position peer groups, age adjustment); ingest nothing from it. |
| 4 | `withqwerty/reep` | **LIVE in GBM — but pinned to the frozen v0 register** (last data release 2026.25, 21 June 2026; 444,707 entities). `resolve.ts` joins it deterministically at confidence 1.000 across 10 providers. **Reep v1 is live at reep.football, CC0, free, 1,703,816 entities (3.8×)** with the biggest gains exactly where GBM is weakest (lower divisions, youth). v0 ids are not interchangeable with v1 ids — but GBM never keyed on them, so migration is a new namespace + re-resolve, not a schema change. **Highest-value free action available.** |
| 5 | `statsbomb/open-data` (the brief's `hudl/open-data` link — Hudl is the parent; the canonical repo is under `statsbomb`) | **Research corpus only.** Best event schema in existence (per-shot xG + freeze-frames, pressures as first-class events, derivable progressive actions) but covers the wrong players (no current major-league seasons; PL most recently 2015/16) and the licence is **non-commercial with mandatory attribution — a human must read `LICENSE.pdf` and rule on GBM's commercial use before any ingestion.** Reep carries no StatsBomb id, so identity would be fuzzy. Correct use: develop and validate the age-adjustment/percentile methodology, never populate the product. The brief's own rule — no fake heatmaps, visual analytics only where real event-location data exists — currently means: **no heatmaps in the product**, because no licensed event feed exists yet. |
| 6 | Kaggle `davidcariboo/player-scores` | **Duplicate of #1** — same author (dcaribou), same dataset, Kaggle is one of its publication channels. GBM already imports from the R2 bucket, which is closer to the source. Nothing to do. |
| 7 | Kaggle `felipesembay/sofascore-and-transfermarkt-football-data` | **Skip.** A static community snapshot; anything Transfermarkt-side is a stale subset of #1. The Sofascore side inherits the Sofascore problem: Opta-downstream data that cannot be sublicensed — a Kaggle re-dump does not launder the rights (and Sofascore's per-match ratings are not comparable with any other provider's, the exact definition-drift trap the API-Football adapter deliberately avoids). Freshness: not maintained; no unique contribution. |
| 8 | Kaggle `saurabhshahane/statsbomb-football-data` | **Skip — use the official repo.** A third-party mirror of #5, older, same licence obligations without the provenance. The brief itself says prefer official sources; correct. |
...
| 4 | **Licence/compliance drift as sources are added.** `source_records` retains payloads permanently by design — some provider licences (BeSoccer's likely; StatsBomb's non-commercial term) may forbid exactly that; API-Football data must never become publicly visible (publication disclaimer). The repo's research flags these; code does not yet enforce any of it. | Medium-High | Record licence terms per provider in `data_providers`; gate ingestion on them; human reads StatsBomb `LICENSE.pdf` before any use. |
...
age-adjustment methodology validated on the StatsBomb corpus *if and only if*
the licence read clears commercial use. Comparison UI once percentiles exist.
No heatmaps until a licensed event-location feed exists — the schema's
honesty rule extends to the charts.
...
- **External sources**: the nine-source research in
  `DATA_SOURCE_RESEARCH.md` re-read in full and cross-checked for internal
  consistency with the adapter code and seeded provider registry; fresh
  re-verification of the load-bearing web claims (Reep v0 freeze / v1
  release, dataset identity of the Kaggle mirrors, StatsBomb licence
  posture) recorded in §5 as of the dates stated there.
